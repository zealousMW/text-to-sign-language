...
class ISLTranslator:
    def __init__(self, video_directory="video", output_directory="output"):
        # Dictionary mapping words to their video files
    ...
    def extract_sov(self, sentence):
        ...
        # Initialize lists for different sentence components
        subjects = []
        objects = []
        verbs = []
        other_words = []
        ...
        if svo_triples:
            for triple in svo_triples:
                # Extract subject, object, and verb
                if triple[0]:  # Subject
        ...
        # Collect words that aren't part of SVO triples
        for token in doc:
        ...
        # Return words in SOV order, followed by other words
        return subjects + objects + verbs + other_words

    def process_sentence(self, sentence):
        """Process a sentence and return a list of video files to play"""
        try:
            # Extract words in SOV order
            words = self.extract_sov(sentence)
            print(f"Words in SOV order: {words}")  # Debug output
            ...
            for word in words:
                if word in self.sign_word_videos:
                    ...
                    # Fingerspell words that aren't in our dictionary
